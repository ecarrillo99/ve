...
const HotelReservation = (props) => {
  ...
  const [date, setDate] = useState([
    {
      ...
      endDate: new Date().setDate(
        new Date().getDate() + parseInt(nNoches ? nNoches : 0)
      ),
      key: "selection",
    },
  ]);
  ...
  return (
    <div className="border mt-3 p-2 flex flex-col rounded-md bg-greenVE-300 ">
      ...
      <label className="font-medium mt-2 text-sm">
      ...
      <span
        className="flex items-center justify-center bg-white  text-sm h-7 z-30 mb-1 border"
      ...
      <button className="bg-greenVE-600 w-full py-1 text-white mt-2">
        Pre-Reservar
      </button>
      <HotelContacts Contactos={establecimiento.Contactos}></HotelContacts>
    </div>
  );
};
