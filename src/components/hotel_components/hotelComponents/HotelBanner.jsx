...
const HotelBanner = (props) => {
  ...
  const handleClickWhatsapp=()=>{
    const message = encodeURIComponent("Descubre las ofertas de "+Establecimiento.Titulo+" en VisitaEcuador.com ingresando aquí: "+shortUrl);
          ...
          <>
            {
              isLoading
                ...
                : (favorito
                  ? <div className="cursor-pointer" dangerouslySetInnerHTML={{ __html: icons.Data.LikeHotel }} onClick={() => handleClickFav()} />
                  : <div className="cursor-pointer"  dangerouslySetInnerHTML={{ __html: icons.Data.UnlikeHotel }} onClick={() => handleClickFav()} />)
            }
          </>
        }
        {
          loadingShare
          ?<div className="w-6 h-6 border-l-2 border-t-2 border-blue-500 border-solid rounded-full animate-spin"></div>
          :<div className="cursor-pointer" onClick={()=>handleClickShare()} dangerouslySetInnerHTML={{ __html: icons.Data.Share}}></div>
        }
