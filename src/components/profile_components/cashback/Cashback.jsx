...
const Cashback = () => {
    ...
    const handleChange = (event) => {
        ...
        if (checked) {
            setTotal(total + parseFloat(historial.find(item => item.id_tbl_red_comision === name).valor))
        } else {
            setTotal(total - parseFloat(historial.find(item => item.id_tbl_red_comision === name).valor))
        }
        ...
        <div className="flex flex-col ">
            ...
            <div className="flex p-3 justify-center">
                <div className='border py-1 px-2 flex items-center gap-2 rounded-l-lg'>
                    ...
                    >{`${formatDate(new Date(dateRange[0].startDate))} al ${formatDate(new Date(dateRange[0].endDate))}`}</span>
                    {openDate && (
                ...
                <button className='bg-greenVE-500 text-white px-2 rounded-r-lg'>Buscar</button>
            </div>

            <div className='flex gap-2'>
                <CashbackTotal setHistorial={setHistorial} setTitle={setTitle} cambiar={cambiar} fechas={date}/>
                <CashbackPorFacturar setHistorial={setHistorial} setTitle={setTitle} cambiar={cambiar} fechas={date}/>
                <CashbackEnProceso setHistorial={setHistorial} setTitle={setTitle} cambiar={cambiar} fechas={date}/>
                <CashbackFacturado setHistorial={setHistorial} setTitle={setTitle} cambiar={cambiar} fechas={date}/>
            </div>
            {
                (title && historial) &&
                <div className={`w-full text-white flex items-center justify-center mb-3 rounded-lg ${title == "Ventas totales"
                    ? "bg-green-700"
                    ...
                    {
                        title == "Ventas por facturar"
                            ? <button className='bg-red-500 px-6 rounded-r-md font-medium' onClick={() => !cargando ? handleClickCobrar() : () => { }}>{!cargando ? "Cobrar $" + total.toFixed(2) : <Spinner ></Spinner>}</button>
                            : <></>
                    }
                </div>
            }
            <div className='flex flex-col gap-3 max-h-80 overflow-auto'>
                {
                    ...
                    historial.map((item) => (
                        <div className='flex flex-col rounded-lg'>
                            <div className={`${item.estado == "EnProceso" ? "bg-[#ffecb4]" : item.estado == "PorFacturar" ? "bg-[#bbdefa]" : "bg-greenVE-100"} flex justify-between px-4 text-black rounded-t-lg text-sm py-1 items-center font-medium`}>
                                <label>
                                    {
                                        item.producto
                                    }
